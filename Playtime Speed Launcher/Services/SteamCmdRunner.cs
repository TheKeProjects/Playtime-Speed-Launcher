...
public static class SteamCmdRunner
{
    ...
    public static async Task RunAsync(
        ...
        Func<string, Task<string?>>? promptAsync = null,
        CancellationToken ct = default)
    {
        ...
        var readTask = ReadOutputWithPromptsAsync(
            proc.StandardOutput, proc.StandardInput, lineOutput, promptAsync, ct);

        await Task.WhenAll(readTask, proc.WaitForExitAsync(ct));
    }

    // Reads stdout one character at a time so we can detect prompts that have no trailing newline.
    private static async Task ReadOutputWithPromptsAsync(
        StreamReader stdout, StreamWriter stdin,
        IProgress<string> lineOutput,
        Func<string, Task<string?>>? promptAsync,
        CancellationToken ct)
    {
        ...
        {
            int n;
            try { n = await stdout.ReadAsync(tmp, 0, 1).WaitAsync(ct); }
            ...
            if (c == '\n')
            {
                ...
                if (!string.IsNullOrWhiteSpace(line))
                    lineOutput.Report(line);
            }
            else
            {
                buf.Append(c);
                var current = buf.ToString();
                if (IsGuardPrompt(current) && promptAsync != null)
                {
                    lineOutput.Report(current);
                    buf.Clear();
                    var code = await promptAsync(current);
                    if (!string.IsNullOrEmpty(code))
                    {
                        await stdin.WriteLineAsync(code);
        ...
        if (buf.Length > 0)
            lineOutput.Report(buf.ToString());
    }

    private static bool IsGuardPrompt(string s)
    {
        var t = s.TrimEnd();
        return t.EndsWith(':') &&
               (t.Contains("Steam Guard", StringComparison.OrdinalIgnoreCase) ||
                t.Contains("Two-factor", StringComparison.OrdinalIgnoreCase) ||
                t.Contains("two factor", StringComparison.OrdinalIgnoreCase));
    }
}
